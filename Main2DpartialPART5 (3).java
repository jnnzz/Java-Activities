...
class Main2DpartialPART5
{
	...
	static void displayOverall(){
	   ...
	   tempArrName = new String[studArr.length][studArr[0].length];
	   tempArrGrades = new double[studArr.length][studArr[0].length];
	   int index = 0;
	   
	   System.out.println();
	   for(int i=0; i<studArr.length; i++){
	       for(int j=0; j<studArr[i].length; j++){
	            tempArrName[i][j] = studArr[i][j].name;
	            tempArrGrades[i][j] = studArr[i][j].getAverage();
	       }
	   }
	    ...
	    for(int i=0; i<studArr.length; i++){
	        for(int j=0; j<studArr[i].length; j++){
	            for(int k=j+1; k<studArr[i].length; k++){
	                if(tempArrGrades[i][j].getAverage()<tempArrGrades[k].getAverage()){
	                    Student temp = tempArrGrades[i][j];
	                    tempArrGrades[i][j] = tempArrGrades[i][k];
	                    tempArrGrades[i][k] = temp;
	                }
	            }
	        }
	    }
	    for(int i=0; i<studArr.length; i++){
	        for(int j=0; j<studArr[0].length; j++){
	            System.out.printf("                       %-6s = %.2f\n",studArr[i][j].name,tempArrGrades[i][j].getAverage());
	        }
	    }
